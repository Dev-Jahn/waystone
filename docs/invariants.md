...
| E-09 | **신뢰·귀속 판정은 파일시스템 메타데이터에 결속하지 않는다** — mtime/ctime/inode/디렉터리 stat/열거 순서는 git으로 전달되지 않고 재파생 불가능하므로 권위의 근거가 될 수 없다. 판정 근거는 파일 내용·파일명·git-tracked 사실에서만 나온다 (근거: round-5 JW-GPT-013 디렉터리 stat→내용 digest, round-6 015 attempt-3의 mtime 단조성 — 같은 실수의 2회 재발) | core/attribution + adapters | characterization + kernel test + fault-injection |
