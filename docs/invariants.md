...
`dev_docs/0.12.0-refactor-plan.md` §4를 권위 원천으로 한다. 확정 문구와 계획 §4가 다르면 이
문서의 확정 문구와 해당 accepted ADR이 우선한다. 집은 계획서 §3·§5와 ADR-0002~0007이 확정한
새 구조의 소유 경계다.
...
| I-10 | 모델에는 목표·경계·판단 요청에 필요한 semantic context와 provenance를 전달하되 bookkeeping protocol은 전달하지 않는다. **protocol 최소화 ≠ 의미 최소화**다. | `jobs/work_brief` + `runs/prompt` | kernel test + focused e2e |
...
## 0.13 intent-promotion 금지 계약

다음 네 자동 전이는 금지한다. 각 경계는 owner/coordinator의 명시적 typed evidence 또는
독립 검증을 요구하며, 대화 요약이나 개수 증가는 증거가 아니다.
...
`PROJECT_BRIEF.md`의 binding fact, WorkBrief provenance, finding validation/disposition,
OutcomeDelta가 각각의 권위를 보존한다.
